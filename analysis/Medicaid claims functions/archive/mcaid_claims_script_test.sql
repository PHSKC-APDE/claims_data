...
select elig.*, claim.mental_dx1_cnt, claim.mental_dxany_cnt, claim.maternal_dx1_cnt, claim.maternal_broad_dx1_cnt,
	...
	claim.mental_dx_rda_any_cnt, claim.sud_dx_rda_any_cnt
...
left join (
	select b.id,
			...
			sum(b.newborn_dx1) as 'newborn_dx1_cnt', sum(b.inpatient) as 'inpatient_cnt', sum( b.ed) as 'ed_cnt',
			sum(b.ed_nohosp) as 'ed_nohosp_cnt', sum(b.ed_avoid_ca) as 'ed_avoid_ca_cnt', sum(b.ed_avoid_ca_nohosp) as 'ed_avoid_ca_nohosp_cnt',
			sum(b.mental_dx_rda_any) as 'mental_dx_rda_any_cnt', sum(b.sud_dx_rda_any) as 'sud_dx_rda_any_cnt'
	...
	from (
		select a.id,
			...
			max(a.newborn_dx1) as 'newborn_dx1', max(a.inpatient) as 'inpatient', max( a.ed) as 'ed',
			max(a.ed_nohosp) as 'ed_nohosp', max(a.ed_avoid_ca) as 'ed_avoid_ca', max(a.ed_avoid_ca_nohosp) as 'ed_avoid_ca_nohosp',
