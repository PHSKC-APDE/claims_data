...
--------------------------------------
--Testing area: Full script using table variables instead of temp tables
--------------------------------------
...
--------------------------
--STEP 1: Table variable for IDs in requested time period
--------------------------
declare @id_t table (id varchar(200))
insert into @id_t (id)
select distinct id from PHClaims.dbo.mcaid_elig_overall where from_date <= @to_date and to_date >= @from_date
 
--------------------------
--STEP 2: Table variable for demo info
--------------------------
declare @demo_t table (id varchar(200), dobnew date, age smallint, age_grp7 varchar(200), gender_mx varchar(200),
	male tinyint, female tinyint, male_t tinyint, female_t tinyint, gender_unk tinyint, race_eth_mx varchar(200), 
	race_mx varchar(200), aian tinyint, asian tinyint, black tinyint, nhpi tinyint, white tinyint, latino tinyint, 
	aian_t tinyint, asian_t tinyint, black_t tinyint, nhpi_t tinyint, white_t tinyint, latino_t tinyint, 
	race_unk tinyint, maxlang varchar(200), english tinyint, spanish tinyint, vietnamese tinyint, chinese tinyint, 
	somali tinyint, russian tinyint, arabic tinyint, korean tinyint, ukrainian tinyint, amharic tinyint, english_t tinyint, 
	spanish_t tinyint, vietnamese_t tinyint, chinese_t tinyint, somali_t tinyint, russian_t tinyint, arabic_t tinyint, 
	korean_t tinyint, ukrainian_t tinyint, amharic_t tinyint, lang_unk tinyint) 

insert into @demo_t (id, dobnew, age, age_grp7, gender_mx,
	male, female, male_t, female_t, gender_unk, race_eth_mx, 
	race_mx, aian, asian, black, nhpi, white, latino, 
	aian_t, asian_t, black_t, nhpi_t, white_t, latino_t, 
	race_unk, maxlang, english, spanish, vietnamese, chinese, 
	somali, russian, arabic, korean, ukrainian, amharic, english_t, 
	spanish_t, vietnamese_t, chinese_t, somali_t, russian_t, arabic_t, 
	korean_t, ukrainian_t, amharic_t, lang_unk) 
...
select x.id, x.dobnew, x.age, 
	...
	from( 	
		...
		where exists (select id from @id_t where id = PHClaims.dbo.mcaid_elig_demoever.id)
		) as x
...
--------------------------
declare @geo_t table (id varchar(200), zip_new float, kcreg_zip varchar(200)) 
insert into @geo_t (id, zip_new, kcreg_zip)
		...
		from (
			...
			from (
				...
				where @from_date <= @to_date and @to_date >= @from_date
					and exists (select id from @id_t where id = PHClaims.dbo.mcaid_elig_address.id)
			) as a
...
--------------------------
declare @cov_t table (id varchar(200), covd smallint, covper decimal(4,1), ccovd_max smallint, covgap_max smallint) 
insert into @cov_t (id, covd, covper, ccovd_max, covgap_max)
...
--------------------------
declare @dual_t table (id varchar(200), duald smallint, dualper decimal(4,1), dual_flag tinyint) 
insert into @dual_t (id, duald, dualper, dual_flag)
...
--1st table - coverage
from (
	select id, covd, covper, ccovd_max, covgap_max from @cov_t
)as cov

--2nd table - dual eligibility duration
inner join (
	select id, duald, dualper, dual_flag from @dual_t
) as dual
on cov.id = dual.id

--3rd table - sub-county areas
inner join (
	select id, zip_new, kcreg_zip from @geo_t
) as geo
--join on ID
on cov.id = geo.id
...
--------------------------------------
--Testing area: Full script using temp tables for demo and geo
--------------------------------------
...
--------------------------
--STEP 4: Query coverage information and join to demographics
--------------------------
...
--------------------------------------
--Testing area: Full script using temp tables for all
--------------------------------------
