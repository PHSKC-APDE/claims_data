...
--1st table - coverage
---------------------------------------
if object_id('tempdb..#cov') IS NOT NULL 
	drop table #cov
...
---------------------------------------
if object_id('tempdb..#geo') IS NOT NULL 
	drop table #geo
...
from (select * from #cov) as cov
inner join (select * from #dual) as dual on cov.id = dual.id
inner join (select * from #geo) as geo on cov.id = geo.id
inner join (select * from #demo) as demo on cov.id = demo.id