...
print("Creating stage.mcaid_claim_line: Data Pull Run time: 00:10:25, Create Index Run Time: 00:05:32")

step1_sql <- glue::glue_sql("
if object_id('[stage].[mcaid_claim_line]', 'U') is not null
drop table [stage].[mcaid_claim_line];", .con = db_claims)


  
)
try(odbc::dbRemoveTable(db_claims, "stage.mcaid_claim_line", temporary = T))
