...
qa_stage_mcaid_claim_procedure <- function(conn = NULL,
                                             conn_qa = NULL,
...
#  }
  ...
  if (DBI::dbExistsTable(conn_qa,
    ...
    num_procedure_current <- DBI::dbGetQuery(
      conn_qa, glue::glue_sql("SELECT YEAR(first_service_date) AS claim_year, COUNT(*) AS current_num_procedure
  ...
  #### SUM UP FAILURES ####
  fail_tot <- sum(ids_fail, procedure_format_fail, num_procedure_fail)# procedure_num_fail
  return(fail_tot)
}
