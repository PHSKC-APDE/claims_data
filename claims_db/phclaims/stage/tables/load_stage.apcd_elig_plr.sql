-- Code to load data to stage.apcd_elig_plr_DATE table
...
--CHANGE NAME OF FINAL TABLE AS NEEDED
--------------------------
...
from (
  ...
  case
    --coverage period fully contains date range
    when from_date <= @from_date and to_date >= @to_date and a.rac_code > 0 then datediff(day, @from_date, @to_date) + 1
    --coverage period begins before and ends within date range
    when from_date <= @from_date and to_date < @to_date and to_date >= @from_date and a.rac_code > 0 then datediff(day, @from_date, to_date) + 1
    --coverage period begins within and ends after date range
    when from_date > @from_date  and to_date >= @to_date and from_date <= @to_date and a.rac_code > 0 then datediff(day, from_date, @to_date) + 1
    --coverage period begins and ends within date range
    when from_date > @from_date and to_date < @to_date and a.rac_code > 0 then datediff(day, from_date, to_date) + 1
    else 0
   end as rac_covd
  from phclaims.final.apcd_elig_timevar as a
  left join phclaims.ref.mcaid_rac_code as b
...
select a.*, b.dsrip_full_covd, b.rac_covd
into #cov3
from (
  ---------
  --MEDICAL variables
...
---------
select c.id, c.geo_zip_code, d.geo_county, e.geo_ach
into #geo
from (
  select b.id, b.geo_zip_code, b.zip_dur, row_number() over (partition by b.id order by b.zip_dur desc, b.geo_zip_code) as zipr
	from (
		select a.id, a.geo_zip_code, sum(a.covd) + 1 as zip_dur
		from (
  		select id_apcd as id, geo_zip_code,
    		case
...
select a.id as id_apcd, 
  ...
  --DEMOGRAPHICS
  b.geo_zip_code, b.geo_county, b.geo_ach, c.geo_ach_covd, 
  ...
  --COVERAGE STATS
  a.med_total_covd, a.med_total_covper, a.dsrip_full_covd, a.rac_covd,
  cast((a.dsrip_full_covd * 1.0) / ((datediff(day, @from_date, @to_date) + 1) * 1.0) * 100.0 as decimal(4,1)) as dsrip_full_covper,
...
case when geo_county is not null then 1 else 0 end as geo_wa_resident,
...
case when geo_county is not null and ((med_medicaid_covper >= 91.7 and rac_covd < med_medicaid_covd) or (dsrip_full_covper >= 91.7 and rac_covd >= med_medicaid_covd))
  and dual_covper < 8.3 and med_commercial_covper < 8.3 then 1 else 0 end as performance_11_wa,
case when geo_county is not null and ((med_medicaid_covper >= 58.3 and rac_covd < med_medicaid_covd) or (dsrip_full_covper >= 58.3 and rac_covd >= med_medicaid_covd))
  and dual_covper < 41.7 and med_commercial_covper < 41.7 then 1 else 0 end as performance_7_wa,
  
--coverage cohorts, ach-level
--same as state-level with addition of ACH-level attribution logic
case when ((med_medicaid_covper >= 91.7 and rac_covd < med_medicaid_covd) or (dsrip_full_covper >= 91.7 and rac_covd >= med_medicaid_covd))
  and dual_covper < 8.3 and med_commercial_covper < 8.3 and geo_ach_covper >= 91.7 then 1 else 0 end as performance_11_ach,
case when ((med_medicaid_covper >= 58.3 and rac_covd < med_medicaid_covd) or (dsrip_full_covper >= 58.3 and rac_covd >= med_medicaid_covd))
  and dual_covper < 41.7 and med_commercial_covper < 41.7 and geo_ach_covper >= 58.3 then 1 else 0 end as performance_7_ach,
geo_zip_code, geo_county, geo_ach, geo_ach_covd, geo_ach_covper, age, age_grp7, gender_me, gender_recent, gender_female, gender_male, med_total_covd, med_total_covper, 
dsrip_full_covd, dsrip_full_covper, dual_covd, dual_covper, dual_flag, med_medicaid_covd, rac_covd, med_medicare_covd, med_commercial_covd,
