#### CODE TO LOAD & TABLE-LEVEL QA STAGE.MCARE_inpatient_base_claims
# Eli Kern, PHSKC (APDE)
#
# 2019-12
...
#### Load script ####
load_stage.mcare_inpatient_base_claims_f <- function() {
  
  ### Run SQL query
  odbc::dbGetQuery(db_claims, glue::glue_sql(
    "--Code to load data to stage.mcare_inpatient_base_claims
--Union of single-year files
    ...
    insert into PHClaims.stage.mcare_inpatient_base_claims_load with (tablock)
    ...
    ,getdate() as last_run
    from PHClaims.load_raw.mcare_inpatient_base_claims_j_14
    ...
    ,icd_prcdr_cd25 as pc25
    ,getdate() as last_run
    from PHClaims.load_raw.mcare_inpatient_base_claims_k
    ...
    ,clm_from_dt as first_service_date
    ,clm_thru_dt as last_service_date
    ,clm_mdcr_non_pmt_rsn as denial_code_facility
    ,nch_clm_type_cd as claim_type
    ,clm_fac_type_cd as facility_type_code
    ,clm_srvc_clsfctn_type_cd as service_type_code
    ,clm_admsn_dt as admission_date
    ...
    ,icd_prcdr_cd25 as pc25
    ,getdate() as last_run
    from PHClaims.load_raw.mcare_inpatient_base_claims_k_17;",
    .con = db_claims))
}

#### Table-level QA script ####
qa_stage.mcare_inpatient_base_claims_qa_f <- function() {
  ...
  row_sum_union <- dbGetQuery(conn = db_claims, glue_sql(
    "select count(*) as qa from PHClaims.stage.mcare_inpatient_base_claims_load;",
    .con = db_claims))
  ...
  col_count <- dbGetQuery(conn = db_claims, glue_sql(
    " select count(*) as col_cnt
      ...
      and table_name = 'mcare_' + 'inpatient_base_claims_load';",
    .con = db_claims))
