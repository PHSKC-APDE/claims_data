...
#2025-01-17 Eli update: Adding CCS, BH and injury columns
...
#### Load script ####
load_stage.mcare_claim_header <- function() {
  ...
  odbc::dbGetQuery(inthealth, glue::glue_sql(
    ...
    --create table shell
    if object_id(N'stg_claims.tmp_mcare_claim_header_ccs',N'U') is not null drop table stg_claims.tmp_mcare_claim_header_ccs;
    create table stg_claims.tmp_mcare_claim_header_ccs (
    	claim_header_id varchar(255) collate SQL_Latin1_General_Cp1_CS_AS,
    ...
    --insert data
    insert into stg_claims.tmp_mcare_claim_header_ccs
    select 
    b.claim_header_id
    ...
    --create table shell
    if object_id(N'stg_claims.tmp_mcare_claim_header_bh',N'U') is not null drop table stg_claims.tmp_mcare_claim_header_bh;
    create table stg_claims.tmp_mcare_claim_header_bh (
    	claim_header_id varchar(255) collate SQL_Latin1_General_Cp1_CS_AS,
        mh_primary tinyint,
    	mh_any tinyint,
    	sud_primary tinyint,
    ...
    --insert data
    insert into stg_claims.tmp_mcare_claim_header_bh
    select 
    b.claim_header_id
    ...
    option (label = 'tmp_mcare_claim_header_bh');
    ...
    if object_id(N'stg_claims.tmp_mcare_claim_header_injury',N'U') is not null drop table stg_claims.tmp_mcare_claim_header_injury;
    create table stg_claims.tmp_mcare_claim_header_injury (
    	claim_header_id varchar(255) collate SQL_Latin1_General_Cp1_CS_AS,
        ecode varchar(255),
    ...
    IF object_id('tempdb..#stage_mcare_icdcm_distinct') is not null drop table #stage_mcare_icdcm_distinct;
    select distinct icdcm_norm, icdcm_version
    into #stage_mcare_icdcm_distinct
    from stg_claims.final_mcare_claim_icdcm_header;
    ...
    IF object_id('tempdb..#stage_mcare_injury_nature_ref') is not null drop table #stage_mcare_injury_nature_ref;
    select distinct *
    into #stage_mcare_injury_nature_ref
    from #stage_mcare_icdcm_distinct
    ...
    IF object_id('tempdb..#stage_mcare_injury_nature') is not null drop table #stage_mcare_injury_nature;
    select a.*,
    ...
    into #stage_mcare_injury_nature
    from stg_claims.final_mcare_claim_icdcm_header as a
    left join #stage_mcare_injury_nature_ref as b
    ...
    IF object_id(N'tempdb..#stage_mcare_injury_cause_icd10cm_ref') is not null drop table #stage_mcare_injury_cause_icd10cm_ref;
    select distinct a.icdcm_norm, a.icdcm_version, b.intent, b.mechanism
    into #stage_mcare_injury_cause_icd10cm_ref
    ...
    IF object_id(N'tempdb..#stage_mcare_injury_cause_icd9cm_ref') is not null drop table #stage_mcare_injury_cause_icd9cm_ref;
    select distinct a.icdcm_norm, a.icdcm_version, b.intent, b.mechanism
    into #stage_mcare_injury_cause_icd9cm_ref
    ...
    IF object_id(N'tempdb..#stage_mcare_injury_cause_ref') is not null drop table #stage_mcare_injury_cause_ref;
    select *
    into #stage_mcare_injury_cause_ref
    from #stage_mcare_injury_cause_icd9cm_ref
    union
    select *
    from #stage_mcare_injury_cause_icd10cm_ref;
    
    --EXACT join of above table to claims data with injury flags
    IF object_id(N'tempdb..#stage_mcare_injury_nature_cause') is not null drop table #stage_mcare_injury_nature_cause;
    select a.*, b.intent, b.mechanism,
    case when b.intent is not null and b.mechanism is not null then 1 else 0 end as ecode_flag
    into #stage_mcare_injury_nature_cause
    from #stage_mcare_injury_nature as a
    left join #stage_mcare_injury_cause_ref as b
    on (a.icdcm_norm = b.icdcm_norm) and (a.icdcm_version = b.icdcm_version);
    
    --Create rank variables for valid external cause-of-injury codes and for nature-of-injury codes
    IF object_id(N'tempdb..#stage_mcare_injury_nature_cause_ranks') is not null drop table #stage_mcare_injury_nature_cause_ranks;
    select *,
    case
    ...
    into #stage_mcare_injury_nature_cause_ranks
    from #stage_mcare_injury_nature_cause;
    
    --Step 10e: Aggregate to claim header level
    
    --Create some aggregated fields
    IF object_id(N'tempdb..#stage_mcare_injury_cause_header_level_tmp') is not null drop table #stage_mcare_injury_cause_header_level_tmp;
    ...
    ecode_rank
    into #stage_mcare_injury_cause_header_level_tmp
    from #stage_mcare_injury_nature_cause_ranks;
    
    --Collapse to claim header level
    IF object_id(N'tempdb..#stage_mcare_injury_cause_header_level_tmp2') is not null drop table #stage_mcare_injury_cause_header_level_tmp2;
    ...
    into #stage_mcare_injury_cause_header_level_tmp2
    from #stage_mcare_injury_cause_header_level_tmp
    ...
    IF object_id(N'tempdb..#stage_mcare_injury_cause_header_level_tmp3') is not null drop table #stage_mcare_injury_cause_header_level_tmp3;
    select a.*, b.icdcm_norm as icdcm_injury_nature, b.icdcm_version as icdcm_injury_nature_version
    into #stage_mcare_injury_cause_header_level_tmp3
    from #stage_mcare_injury_cause_header_level_tmp2 as a
    left join (select * from #stage_mcare_injury_nature_cause_ranks where injury_nature_rank = 1) as b
    on a.claim_header_id = b.claim_header_id;
    ...
    IF object_id(N'tempdb..#stage_mcare_distinct_injury_nature_icdcm_tmp1') is not null drop table #stage_mcare_distinct_injury_nature_icdcm_tmp1;
    ...
    IF object_id(N'tempdb..#stage_mcare_distinct_injury_nature_icdcm_final') is not null drop table #stage_mcare_distinct_injury_nature_icdcm_final;
    ...
    into #stage_mcare_distinct_injury_nature_icdcm_final
    from #stage_mcare_distinct_injury_nature_icdcm_tmp1;
    
    --Step 10g: Add broad type categories to nature of injury ICD-CM codes
    insert into stg_claims.tmp_mcare_claim_header_injury
    select
    a.*,
    b.ccs_detail_desc as icdcm_injury_nature_type
    from #stage_mcare_injury_cause_header_level_tmp3 as a
    left join #stage_mcare_distinct_injury_nature_icdcm_final as b
    on (a.icdcm_injury_nature = b.icdcm_injury_nature) and (a.icdcm_injury_nature_version = b.icdcm_injury_nature_version)
    option (label = 'tmp_mcare_claim_header_injury');
    ...
    on a.claim_header_id = b.claim_header_id
    left join stg_claims.tmp_mcare_claim_header_ccs as c
    on a.claim_header_id = c.claim_header_id
    left join stg_claims.tmp_mcare_claim_header_bh as d
    on a.claim_header_id = d.claim_header_id
...
qa_stage.mcare_claim_header_qa <- function() {
