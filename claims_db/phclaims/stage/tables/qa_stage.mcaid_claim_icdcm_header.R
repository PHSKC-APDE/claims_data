...
last_run <- as.POSIXct(DBI::dbGetQuery(
  db_claims, "SELECT MAX (last_run) FROM claims.stage_mcaid_claim_icdcm_header")[[1]])
...
ids_demo_chk <- as.integer(DBI::dbGetQuery(db_claims,
  ...
  FROM claims.stage_mcaid_claim_icdcm_header AS a
...
FROM claims.stage_mcaid_claim_icdcm_header WHERE icdcm_version = 9")
...
"SELECT MIN(LEN(icdcm_norm)) as min_len, MAX(LEN(icdcm_norm)) as max_len 
FROM claims.stage_mcaid_claim_icdcm_header WHERE icdcm_version = 10")
...
from claims.stage_mcaid_claim_icdcm_header
...
dx_chk <- as.integer(DBI::dbGetQuery(db_claims,
  ...
  FROM claims.stage_mcaid_claim_icdcm_header as a
  WHERE not exists
  (SELECT 1 FROM claims.ref_dx_lookup as b
...
if (DBI::dbExistsTable(db_claims,
                       DBI::Id(schema = "claims", table = "final_mcaid_claim_icdcm_header"))) {
