...
# This code QAs table [stage].[mcaid_claim_line]
#
...
last_run <- as.POSIXct(DBI::dbGetQuery(
  db_claims, "SELECT MAX (last_run) FROM stage.mcaid_claim_line")[[1]])
...
rows_line <- as.integer(odbc::dbGetQuery(
  conn = db_claims, "SELECT COUNT(DISTINCT [claim_line_id]) FROM [stage].[mcaid_claim_line]"))
rows_raw <- as.integer(odbc::dbGetQuery(
  conn = db_claims, "SELECT COUNT(DISTINCT [CLM_LINE_TCN]) FROM [stage].[mcaid_claim]"))
...
if (rows_line == rows_raw) {
  ...
  DBI::dbExecute(conn = db_claims,
                 glue::glue_sql("INSERT INTO metadata.qa_mcaid
                   ...
                   'FAIL', 
                   {Sys.time()}, 
                   '[stage].[mcaid_claim_line] had {rows_line} distinct claim lines ", 
                   "compared to {rows_raw} in [stage].[mcaid_claim]')",
                                .con = db_claims))
}
...
rac_chk <- as.integer(DBI::dbGetQuery(
  ...
  WHERE NOT EXISTS
  (
    SELECT 1 FROM [ref].[mcaid_rac_code] as b
