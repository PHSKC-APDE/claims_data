...
qa_stage_mcaid_claim_moud <- function(conn = NULL,
                                        conn_qa = NULL,
  ...
  #### Check for new NDCs ####
  ndc_cnt <- DBI::dbGetQuery(conn, glue::glue_sql("select * from {`stage_schema`}.tmp_mcaid_moud_pharm_1 where admin_method is null ", .con = conn))
  ...
  #### SUM UP FAILURES ####
  DBI::dbExecute(conn,
                 glue::glue_sql("DROP TABLE {`stage_schema`}.tmp_mcaid_moud_pharm_1",
                                .con = conn))
