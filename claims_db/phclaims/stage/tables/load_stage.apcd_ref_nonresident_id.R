...
load_stage.apcd_ref_nonresident_id_f <- function() {
  odbc::dbGetQuery(dw_inthealth, glue::glue_sql(
    ...
    with temp1 as (
      select a.id_apcd, a.year_month, case when b.zip_group_desc is null then 1 else 0 end as out_of_state
      from (
        select internal_member_id as id_apcd, year_month, zip_code
        from stg_claims.apcd_member_month_detail
      ) as a
      left join (select * from stg_claims.ref_apcd_zip_group where zip_group_type_desc = 'County') as b
