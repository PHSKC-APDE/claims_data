...
if object_id(N'claims.tmp_apcd_claim_header_temp1',N'U') is not null drop table claims.tmp_apcd_claim_header_temp1;
...
into claims.tmp_apcd_claim_header_temp1
from claims.stage_apcd_medical_claim_header_cci as a
...
left join (
    ...
    from claims.final_apcd_claim_line
...
left join claims.ref_apcd_claim_status as c
...
-------------------
if object_id(N'claims.tmp_apcd_claim_header_ed_procedure_code',N'U') is not null drop table claims.tmp_apcd_claim_header_ed_procedure_code;
select x.medical_claim_header_id, x.ed_procedure_code_temp
into claims.tmp_apcd_claim_header_ed_procedure_code
from (
    ...
    left join claims.final_apcd_claim_procedure as b
    on a.medical_claim_header_id = b.claim_header_id
    --exclude denined/orphaned claims
    where a.denied_header_flag = 'N' and a.orphaned_header_flag = 'N'
...
-------------------
if object_id(N'claims.tmp_apcd_claim_header_pc_visit',N'U') is not null drop table claims.tmp_apcd_claim_header_pc_visit;
select x.medical_claim_header_id, x.pc_procedure_temp, x.pc_taxonomy_temp, x.pc_zcode_temp
into claims.tmp_apcd_claim_header_pc_visit
from (
select a.medical_claim_header_id,
...
left join (
    ...
    --procedure code table
    from claims.final_apcd_claim_procedure as b1
    --primary care-relevant procedure codes
    inner join (select code from claims.ref_pc_visit_oregon where code_system in ('cpt', 'hcpcs')) as b2
...
left join (
    ...
    --ICD-CM table
    from claims.final_apcd_claim_icdcm_header as c1
    --primary care-relevant ICD-10-CM codes
    inner join (select code from claims.ref_pc_visit_oregon where code_system = 'icd10cm') as c2
...
left join (
    ...
    from (select * from claims.final_apcd_claim_provider where provider_type in ('rendering', 'attending')) as d1
    --NPIs for each provider
    inner join claims.ref_apcd_provider_npi as d2
    on d1.provider_id_apcd = d2.provider_id_apcd
    --taxonomy codes for rendering and attending providers
    inner join claims.ref_kc_provider_master as d3
    on d2.npi = d3.npi
    --primary care-relevant provider taxonomy codes
    inner join (select code from claims.ref_pc_visit_oregon where code_system = 'provider_taxonomy') as d4
...
------------------
if object_id(N'claims.tmp_apcd_claim_header_icd1',N'U') is not null drop table claims.tmp_apcd_claim_header_icd1;
select claim_header_id,
min(icdcm_norm) as primary_diagnosis,
min(icdcm_version) as icdcm_version
into claims.tmp_apcd_claim_header_icd1
from claims.final_apcd_claim_icdcm_header
...
--create table shell
if object_id(N'claims.tmp_apcd_claim_header_temp1b',N'U') is not null drop table claims.tmp_apcd_claim_header_temp1b;
create table claims.tmp_apcd_claim_header_temp1b (
...
insert into claims.tmp_apcd_claim_header_temp1b with (tablock)
select
	a.claim_header_id,
	b.primary_diagnosis, b.icdcm_version,
	c.ed_procedure_code_temp,
	d.pc_procedure_temp, d.pc_taxonomy_temp, d.pc_zcode_temp
from (select distinct claim_header_id from claims.tmp_apcd_claim_header_temp1) as a
left join claims.tmp_apcd_claim_header_icd1 as b
on a.claim_header_id = b.claim_header_id
left join claims.tmp_apcd_claim_header_ed_procedure_code as c
on a.claim_header_id = c.medical_claim_header_id
left join claims.tmp_apcd_claim_header_pc_visit as d
...
--create table shell
if object_id(N'claims.tmp_apcd_claim_header_temp2',N'U') is not null drop table claims.tmp_apcd_claim_header_temp2;
create table claims.tmp_apcd_claim_header_temp2 (
id_apcd bigint,
claim_header_id bigint,
...
--insert into table shell with tablock
insert into claims.tmp_apcd_claim_header_temp2 with (tablock)
...
from claims.tmp_apcd_claim_header_temp1 as a
left join (select * from claims.ref_kc_claim_type_crosswalk where source_desc = 'apcd') as b
on a.claim_type_apcd_id = b.source_clm_type_id
left join claims.tmp_apcd_claim_header_temp1b as c
...
-------------------
if object_id(N'claims.tmp_apcd_claim_header_temp3',N'U') is not null drop table claims.tmp_apcd_claim_header_temp3;
select *,
--primary care visits
...
into claims.tmp_apcd_claim_header_temp3
from claims.tmp_apcd_claim_header_temp2;
...
if object_id(N'claims.tmp_apcd_claim_header_ed_yale_1',N'U') is not null drop table claims.tmp_apcd_claim_header_ed_yale_1;
select id_apcd, claim_header_id, first_service_date, last_service_date, 'Carrier' as ed_type
into claims.tmp_apcd_claim_header_ed_yale_1
from claims.tmp_apcd_claim_header_temp3
where ed_yale_carrier = 1
    
union
select id_apcd, claim_header_id, first_service_date, last_service_date, 'Outpatient' as ed_type
from claims.tmp_apcd_claim_header_temp3 where ed_yale_opt = 1
    
union
select id_apcd, claim_header_id, first_service_date, last_service_date, 'Inpatient' as ed_type
from claims.tmp_apcd_claim_header_temp3 where ed_yale_ipt = 1;
...
if object_id(N'claims.tmp_apcd_claim_header_ed_yale_final',N'U') is not null drop table claims.tmp_apcd_claim_header_ed_yale_final;
...
,CASE WHEN ROW_NUMBER() OVER(PARTITION BY [id_apcd] 
    ...
    END AS [increment]
FROM claims.tmp_apcd_claim_header_ed_yale_1
...
INTO claims.tmp_apcd_claim_header_ed_yale_final
...
-------------------
insert into claims.stage_apcd_claim_header with (tablock)
select distinct
a.id_apcd,
a.claim_header_id,
a.product_code_id,
...
a.pc_visit_id,
getdate() as last_run
from claims.tmp_apcd_claim_header_temp3 as a
left join claims.tmp_apcd_claim_header_ed_yale_final as b
on a.claim_header_id = b.claim_header_id;
...
qa_stage.apcd_claim_header_f <- function() {
  ...
  res1 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', '# of non-distinct headers, expect 0' as qa_type,
    count(a.claim_header_id) as qa1, qa2 = null
    from (
      select claim_header_id, count(*) as header_cnt
      from claims.stage_apcd_claim_header
      group by claim_header_id
    ) as a
  ...
  res2 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', 'qa1 = distinct IDs, qa2 = distinct headers' as qa_type,
    count(distinct id_apcd) as qa1, count(distinct claim_header_id) as qa2
    from claims.stage_apcd_claim_header;",
    .con = db_claims))
  
  res3 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_medical_claim_header' as 'table', 'qa1 = distinct IDs, qa2 = distinct headers' as qa_type,
    count(distinct internal_member_id) as qa1, count(distinct medical_claim_header_id) as qa2
    from claims.stage_apcd_medical_claim_header
    --exclude denined/orphaned claims
  ...
  res4 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', '# of members not in elig_demo, expect 0' as qa_type,
    count(a.id_apcd) as qa1, qa2 = null
    from claims.stage_apcd_claim_header as a
  ...
  res5 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', '# of members not in elig_timevar, expect 0' as qa_type,
    count(a.id_apcd) as qa1, qa2 = null
    from claims.stage_apcd_claim_header as a
  ...
  res6 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', '# of claims with unmatched claim type, expect 0' as qa_type,
  ...
  res7 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', '# of ipt stays with no discharge date, expect 0' as qa_type,
    count(*) as qa1, qa2 = null
    from claims.stage_apcd_claim_header
  ...
  res8 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', '# of ed_pophealth_id values used for >1 person, expect 0' as qa_type,
  ...
  res9 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', 'qa1 = distinct ed_pophealth_id, qa2 = max - min + 1' as qa_type,
  ...
  res10 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_header' as 'table', '# of ed_perform rows with no ed_pophealth, expect 0' as qa_type,
    count(*) as qa1, qa2 = null
    from claims.stage_apcd_claim_header
  ...
  res11 <- dbGetQuery(conn = db_claims, glue_sql(
    ...
    select 'claims.stage_apcd_claim_header' as 'table', '# of ed_pophealth visits where the overlap date is greater than 1 day, expect 0' as 'qa_type',
      count(*) as qa1, qa2 = null
    from claims.stage_apcd_claim_header
