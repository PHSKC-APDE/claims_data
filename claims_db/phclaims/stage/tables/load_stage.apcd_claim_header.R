...
into PHClaims.tmp.apcd_claim_header_temp1
...
into PHClaims.tmp.apcd_claim_header_ed_procedure_code
from (
...
-------------------
if object_id(N'PHClaims.tmp.apcd_claim_header_pc_visit',N'U') is not null drop table PHClaims.tmp.apcd_claim_header_pc_visit;
select x.medical_claim_header_id, x.pc_procedure_temp, x.pc_taxonomy_temp, x.pc_zcode_temp
into PHClaims.tmp.apcd_claim_header_pc_visit
from (
select a.medical_claim_header_id,
...
into PHClaims.tmp.apcd_claim_header_icd1
...
--create table shell
if object_id(N'PHClaims.tmp.apcd_claim_header_temp2',N'U') is not null drop table PHClaims.tmp.apcd_claim_header_temp2;
create table PHClaims.tmp.apcd_claim_header_temp2 (
id_apcd bigint,
claim_header_id bigint,
...
insert into PHClaims.tmp.apcd_claim_header_temp2 with (tablock)
...
left join PHClaims.tmp.apcd_claim_header_temp1b as c
...
-------------------
if object_id(N'PHClaims.tmp.apcd_claim_header_temp3',N'U') is not null drop table PHClaims.tmp.apcd_claim_header_temp3;
select *,
--primary care visits
...
into PHClaims.tmp.apcd_claim_header_temp3
from PHClaims.tmp.apcd_claim_header_temp2;
...
if object_id(N'PHClaims.tmp.apcd_claim_header_ed_yale_1',N'U') is not null drop table PHClaims.tmp.apcd_claim_header_ed_yale_1;
select id_apcd, claim_header_id, first_service_date, last_service_date, 'Carrier' as ed_type
into PHClaims.tmp.apcd_claim_header_ed_yale_1
from PHClaims.tmp.apcd_claim_header_temp3
where ed_yale_carrier = 1
    
union
select id_apcd, claim_header_id, first_service_date, last_service_date, 'Outpatient' as ed_type
from PHClaims.tmp.apcd_claim_header_temp3 where ed_yale_opt = 1
    
union
select id_apcd, claim_header_id, first_service_date, last_service_date, 'Inpatient' as ed_type
from PHClaims.tmp.apcd_claim_header_temp3 where ed_yale_ipt = 1;
...
if object_id(N'PHClaims.tmp.apcd_claim_header_ed_yale_final',N'U') is not null drop table PHClaims.tmp.apcd_claim_header_ed_yale_final;
...
,CASE WHEN ROW_NUMBER() OVER(PARTITION BY [id_apcd] 
    ...
    END AS [increment]
FROM PHClaims.tmp.apcd_claim_header_ed_yale_1
...
INTO PHClaims.tmp.apcd_claim_header_ed_yale_final
...
a.pc_visit_id,
getdate() as last_run
from PHClaims.tmp.apcd_claim_header_temp3 as a
left join PHClaims.tmp.apcd_claim_header_ed_yale_final as b
on a.claim_header_id = b.claim_header_id;
