...
load_stage_mcaid_elig_timevar_f <- function(conn = NULL,
  ...
  ## The z sub query consolidates duplicated rows with different DUALELIGIBLE_INDICATOR columns to take either DualEligible or PartialDual
  step1a_sql <- glue::glue_sql(paste0(
    ...
    (SELECT MBR_H_SID AS 'id_mcaid', 
      ...
      CASE WHEN MIN(DUALELIGIBLE_INDICATOR) = 'N/A' THEN MAX(DUALELIGIBLE_INDICATOR) ELSE MIN(DUALELIGIBLE_INDICATOR) END AS 'dual', 
      ...
      GROUP BY MBR_H_SID, CLNDR_YEAR_MNTH, RAC_FROM_DATE, RAC_TO_DATE, 
        HEALTH_HOME_CLINICAL_INDICATOR, RAC_CODE, RPRTBL_BSP_GROUP_CID, 
        COVERAGE_TYPE_IND, MC_PRVDR_ID, geo_hash_raw) a
      LEFT JOIN
      (SELECT rac_code, 
        CASE 
  ...
  (SELECT id_mcaid, dual, health_home_flag, bsp_group_cid, full_benefit, cov_type, mco_id,
    ...
    CASE 
      ...
      WHEN startdate >= fromdate THEN startdate
         WHEN startdate < fromdate
              AND startdate = lag(startdate, 1) over(partition by id_mcaid, startdate order by fromdate)
              AND lag(fromdate, 1) over(partition by id_mcaid, startdate order by fromdate) <= startdate
              THEN startdate
    ...
    CASE 
      ...
      WHEN enddate <= todate THEN enddate
         WHEN enddate > todate
              AND enddate = lead(enddate, 1) over (partition by id_mcaid, enddate order by todate)
              AND lead(todate, 1) over (partition by id_mcaid, enddate order by todate) >= enddate
              THEN enddate
