...
load_stage.apcd_elig_plr_f <- function(from_date = NULL, to_date = NULL, calendar_year = T, table_name = NULL) {
  ...
  if (calendar_year == T) {
    ...
    table_name_year <- glue::glue_sql(paste0("apcd_elig_plr_", table_name_year), .con = dw_inthealth)
  }
  
  if (calendar_year == F) {
    table_name_year <- glue::glue_sql(paste0("apcd_elig_plr_", table_name), .con = dw_inthealth)
  }
