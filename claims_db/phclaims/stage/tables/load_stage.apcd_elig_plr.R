...
qa_stage.apcd_elig_plr_f <- function(year = NULL) {
  ...
  res1 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# members with >1 row, expect 0' as qa_type, 
      select count(a.id_apcd) as qa
        from (
  ...
  res2 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', 'in state but not ach 11-month cohort, expect < 91.7' as qa_type,
      select max(geo_ach_covper) as qa
      from stage.{`table_name`}
  ...
  res3 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', 'in state but not ach 7-month cohort, expect <58.3' as qa_type,
      select max(geo_ach_covper) as qa
      from stage.{`table_name`}
  ...
  res4 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', 'non-WA county for WA resident, expect 0' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
  ...
  res5 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', 'non-WA residents, expect 0' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
  ...
  res6 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of overall Medicaid members' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
  ...
  res7 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of members with medical but not pharmacy Medicaid' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
  ...
  res8 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of members with pharmacy but not medical Medicaid, expect low' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
  ...
  res9 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of members with day counts >365, expect 0' as qa_type, 
      select count(*) as qa from stage.{`table_name`}
  ...
  res10 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of members with percents >100, expect 0' as qa_type, 
      select count(*) as qa from stage.{`table_name`}
  ...
  res11 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of overall Medicaid members out of state, expect 0' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
  ...
  res12 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of 11-month cohort members out of state, expect 0' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
  ...
  res13 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'stage.{`table_name`}' as 'table', '# of 7-month cohort members out of state, expect 0' as qa_type, 
      select count(id_apcd) as qa
      from stage.{`table_name`}
