...
load_stage.apcd_claim_procedure_f <- function() {
  ...
  odbc::dbGetQuery(db_claims, glue::glue_sql(
    ...
    -------------------
    insert into claims.stage_apcd_claim_procedure with (tablock)
    ...
    from claims.stage_apcd_claim_procedure_raw_cci as a
...
qa_stage.apcd_claim_procedure_f <- function() {
  ...
  res1 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_procedure' as 'table', '# members not in elig_demo, expect 0' as qa_type,
    count(distinct a.id_apcd) as qa
    from claims.stage_apcd_claim_procedure as a
  ...
  res2 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_procedure' as 'table', '# members not in elig_timevar, expect 0' as qa_type,
    count(distinct a.id_apcd) as qa
    from claims.stage_apcd_claim_procedure as a
  ...
  res3 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_procedure' as 'table', '# of null procedure codes, expect 0' as qa_type,
    count(*) as qa
    from claims.stage_apcd_claim_procedure
