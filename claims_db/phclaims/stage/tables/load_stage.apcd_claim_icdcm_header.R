...
load_stage.apcd_claim_icdcm_header_f <- function() {
  ...
  odbc::dbGetQuery(db_claims, glue::glue_sql(
    ...
    -------------------
    insert into claims.stage_apcd_claim_icdcm_header with (tablock)
    ...
    from claims.stage_apcd_claim_icdcm_raw_cci as a
...
qa_stage.apcd_claim_icdcm_header_f <- function() {
  ...
  res1 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', '# members not in elig_demo, expect 0' as qa_type,
    count(distinct a.id_apcd) as qa
    from claims.stage_apcd_claim_icdcm_header as a
  ...
  res2 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', '# members not in elig_timevar, expect 0' as qa_type,
    count(distinct a.id_apcd) as qa
    from claims.stage_apcd_claim_icdcm_header as a
  ...
  res3 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', 'minimum length of ICD-9-CM, expect 5' as qa_type,
    min(len(icdcm_norm)) as qa
    from claims.stage_apcd_claim_icdcm_header
    where icdcm_version = 9;",
    .con = db_claims))
  
  res4 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', 'maximum length of ICD-9-CM, expect 5' as qa_type,
  ...
  res5 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', '# of null diagnoses, expect 0' as qa_type,
    count(*) as qa
    from claims.stage_apcd_claim_icdcm_header
  ...
  res8 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', '# of diagnoses not joining, expect <100' as qa_type,
  ...
  res9 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', 'minimum length of ICD-10-CM, expect >=3' as qa_type,
    min(len(icdcm_norm)) as qa
    from claims.stage_apcd_claim_icdcm_header
  ...
  res10 <- dbGetQuery(conn = db_claims, glue_sql(
    "select 'claims.stage_apcd_claim_icdcm_header' as 'table', 'maximum length of ICD-10-CM, expect <=7' as qa_type,
    max(len(icdcm_norm)) as qa
    from claims.stage_apcd_claim_icdcm_header
    where icdcm_version = 10;",
    .con = db_claims))
