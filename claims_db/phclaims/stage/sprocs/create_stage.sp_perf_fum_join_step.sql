...
/*
This procedure join FUM ED visits (denominator events) with their follow-up 
visits (numerator events).

(1) Call [stage].[fn_perf_fum_ed_index_visit_exclusion] to get ED visits 
subject to:
...
(2) Call [stage].[fn_perf_fum_follow_up_visit] to get follow-up visits
...
IF OBJECT_ID('[stage].[sp_perf_fum_join_step]', 'P') IS NOT NULL
DROP PROCEDURE [stage].[sp_perf_fum_join_step];
GO
CREATE PROCEDURE [stage].[sp_perf_fum_join_step]
...
/*
We can use [stage].[fn_perf_fua_ed_index_visit_exclusion] for FUM Measure
with @age=6, @dx_value_set_name=''Mental Illness''
*/
...
FROM [stage].[fn_perf_fum_follow_up_visit]('''
+ CAST(@measurement_start_date AS VARCHAR(200)) + ''', ''' 
+ CAST(@measurement_end_date AS VARCHAR(200)) + ''');
