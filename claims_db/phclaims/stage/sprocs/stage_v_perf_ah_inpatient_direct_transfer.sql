...
,[first_service_date] AS [claim_from_date]
,[last_service_date] AS [claim_last_service_date]
,[patient_status]
,[date_diff]
,[increment]
,[episode_id]
,FIRST_VALUE([first_service_date]) OVER(PARTITION BY [id_mcaid], [episode_id] ORDER BY [first_service_date], [last_service_date], [claim_header_id] ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS [episode_from_date]
,LAST_VALUE([last_service_date]) OVER(PARTITION BY [id_mcaid], [episode_id] ORDER BY [from_date], [last_service_date], [claim_header_id] ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS [episode_last_service_date]
,COUNT(*) OVER(PARTITION BY [id_mcaid], [episode_id] ORDER BY [id_mcaid], [episode_id], [claim_header_id]) AS [count_stays]
--,ROW_NUMBER() OVER(PARTITION BY [id_mcaid], [episode_id] ORDER BY [first_service_date] DESC, [last_service_date] DESC, [claim_header_id] DESC) AS [stay_id]
...
,MAX(CASE WHEN [patient_status] = 'Expired' THEN 1 ELSE 0 END) OVER(PARTITION BY [id_mcaid], [episode_id] ORDER BY [id_mcaid], [episode_id]) AS [death_during_stay]
