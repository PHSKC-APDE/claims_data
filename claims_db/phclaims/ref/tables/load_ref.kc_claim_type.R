##Code to create and load data to ref.kc_claim_type
##Reference table for King County defined claim type
##Eli Kern (PHSKC-APDE)
##2019-4-26
...
create_table_f(conn = db.claims51, 
               config_file = file.path(git_path, "claims_data/claims_db/phclaims/ref/tables", "create_ref.kc_claim_type.yaml"),
               overall = T, ind_yr = F, test_mode = F)

#### STEP 2: Load data from CSV ####
load_table_from_file_f(conn = db.claims51, 
                       config_file = file.path(git_path, "claims_data/claims_db/phclaims/ref/tables", "load_ref.kc_claim_type.yaml"),
                       overall = T, ind_yr = F, test_mode = F)
