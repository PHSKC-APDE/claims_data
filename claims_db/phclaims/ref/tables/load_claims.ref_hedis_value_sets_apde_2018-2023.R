...
system.time(dbWriteTable(db_claims, name = DBI::Id(schema = to_schema, table = to_table_measures), 
             value = as.data.frame(hedis_measures), 
             overwrite = T))

#HEDIS value sets (69 min)
system.time(dbWriteTable(db_claims, name = DBI::Id(schema = to_schema, table = to_table_value_sets), 
             value = as.data.frame(hedis_value_sets), 
             overwrite = T))

#HEDIS medication measures (<1 min)
system.time(dbWriteTable(db_claims, name = DBI::Id(schema = to_schema, table = to_table_med_measures), 
             value = as.data.frame(hedis_medication_measures), 
             overwrite = T))

#HEDIS medication lists (225 min)
system.time(dbWriteTable(db_claims, name = DBI::Id(schema = to_schema, table = to_table_med_lists), 
             value = as.data.frame(hedis_medication_lists), 
             overwrite = T))

# Add index on HEDIS value sets table (<1 min)
system.time(DBI::dbExecute(db_claims, 
  glue::glue_sql(
