#### CODE TO LOAD APCD PHARMACY_CLAIM TABLES
# Eli Kern, PHSKC (APDE)
#
# 2019-10
...
load_load_raw.apcd_pharmacy_claim_full_f <- function(etl_date_min = NULL,
                                                               etl_date_max = NULL,
  ...
  odbc::dbGetQuery(db_claims,
                   glue::glue_sql(
                     "ALTER TABLE load_raw.apcd_pharmacy_claim
  ...
  if (length(table_config$years) > 1) {
    lapply(table_config$years, function(x) {
      table_name <- glue::glue("apcd_pharmacy_claim_", x)
