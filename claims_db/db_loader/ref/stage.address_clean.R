...
manual_add <- manual_add %>% mutate(geo_zip_raw = case_when(
  nchar(geo_zip_raw) == 4 ~ as.character(paste0("0", geo_zip_raw)),
  TRUE ~ as.character(geo_zip_raw)),
...
mcaid_add <- dbGetQuery(db_claims51, 
                        "SELECT DISTINCT 
...
### Bring in public housing authority addresses
pha_add <- dbGetQuery(db_apde51, 
...
# Set missing add to NA to match Medicaid
pha_add <- pha_add %>% 
...
joint_add <- inner_join(select(mcaid_add, geo_add1_raw, geo_add2_raw, 
                        ...
                        by = c("geo_add1_raw", "geo_add2_raw", 
                               "geo_city_raw", "geo_state_raw", "geo_zip_raw")) %>%
  left_join(., select(mcaid_add, geo_add1_raw, geo_add2_raw, 
                      geo_city_raw, geo_state_raw, geo_zip_raw),
            by = c("geo_add1_raw", "geo_add2_raw", "geo_city_raw", "geo_state_raw", "geo_zip_raw"))



# Remove common addresses from source
mcaid_add <- anti_join(mcaid_add, 
...
### Combine files
combined_add <- bind_rows(mcaid_add, pha_add, joint_add)
...
combined_add_in_clean <- combined_add_in_clean %>%
  ...
  old_add1 = case_when(
    ...
    TRUE ~ old_add1),
  old_zip = case_when(
    nchar(old_zip) == 4 ~ as.character(paste0("0", old_zip)),
    TRUE ~ as.character(old_zip)
    ),
...
combined_add_full_load <- combined_add_full %>%
  distinct(geo_add1_raw, geo_add2_raw, geo_city_raw, geo_state_raw, geo_zip_raw,
...
dbWriteTable(db_claims51, tbl_id_meta, combined_add_full_load, overwrite = T)
...
geocoded_2018_06_12 <- geocoded_2018_06_120 %>%
  as.data.frame() %>%
