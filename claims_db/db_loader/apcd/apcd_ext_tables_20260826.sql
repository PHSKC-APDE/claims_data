''
CREATE EXTERNAL TABLE "claims"."stage_apcd_elig_plr_2025"
  ([id_apcd] BIGINT NULL, 
  [geo_wa] TINYINT NULL, 
...
WITH (DATA_SOURCE = [datascr_WS_EDW], SCHEMA_NAME = N'stg_claims', OBJECT_NAME = N'stage_apcd_elig_plr_2025');

''
CREATE EXTERNAL TABLE "claims"."stage_apcd_claim_bh"
  ([id_apcd] BIGINT NULL, 
  [first_encounter_date] DATE NULL, 
  [last_encounter_date] DATE NULL, 
  [bh_cond] VARCHAR(200) COLLATE SQL_Latin1_General_CP1_CI_AS NULL, 
  [last_run] DATE NULL)
WITH (DATA_SOURCE = [datascr_WS_EDW], SCHEMA_NAME = N'stg_claims', OBJECT_NAME = N'stage_apcd_claim_bh');

IF OBJECT_ID('"claims"."stage_apcd_claim_icdcm_header"') IS NOT NULL
DROP EXTERNAL TABLE "claims"."stage_apcd_claim_icdcm_header";
