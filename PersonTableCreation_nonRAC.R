...
# Manually run code in step 7. Need to turn this into a function to save space
...
elig <- elig %>%
  group_by(id, ssnnew) %>%
  mutate(check = ifelse(tofinal >= lead(fromfinal, 1) &
                          !is.na(lead(fromfinal, 1)), 1, 0)) %>%
  ungroup()
...
elig.tst <- elig %>%
  ungroup() %>%
  filter(row_number() <= 100000 | year(todate) == "2105")


# Fix up dates
elig.tst$todate <- as.POSIXlt(elig.tst$todate)
elig.tst$todate <- as.Date(elig.tst$todate)

elig.tst <- elig.tst %>%
  mutate(
    todatenew = as.Date(ifelse(
      todate == "2999-10-31", "2999-12-31", todate
    ), origin = "1970-01-01"),
    # Separate out date components in order to fix year typos
    todateyear = year(todate),
    todatemonth = month(todate),
  ...
  # Remove remaining erroneous todates (currently, this also removes that person's record
  # as that is their only entry in the table, ~8 people affected)
  filter(todatenew <= "2019-12-31" | todatenew >= "2999-12-31")
...
#### Save cleaned person table (preserves all rows) ####
elig.upload <- elig %>%
  select(year:cntyname, ssnnew, citynew, homeless:gendernew, race1new:racehnum)

ptm02 <- proc.time() # Times how long this query takes
sqlDrop(db.apde, "dbo.medicaidPerTblFull")
sqlSave(
  db.apde,
  elig.upload,
  tablename = "dbo.medicaidPerTblFull",
  varTypes = c(fromdate = "Date",
               todate = "Date")
)
proc.time() - ptm02
